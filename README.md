...
Automatically **compress, resize and convert your WordPress Media Library** to WebP/AVIF/JPEG XL at Squoosh-level quality — like Smush / ShortPixel / Imagify, but with the compression running **client-side in WebAssembly** instead of a paid cloud or your host's PHP image library. That means elite quality (mozjpeg, oxipng, WebP, AVIF, JPEG XL) that works identically on **every** host, with **nothing ever leaving your site** and **no per-image cost**.
...
The server **never encodes an image**. All decode/resize/flatten/encode happens in a Web Worker in the browser using the [`@jsquash`](https://github.com/jamsinclair/jSquash) WebAssembly codecs (derived from Squoosh). The browser picks whichever candidate (recompressed original, WebP, AVIF, or JPEG XL) is smallest and sends only that one back. WordPress-side PHP only:
...
**Trade-off:** replacing a PNG/JPEG with WebP/AVIF/JPEG XL means very old browsers (IE11, iOS ≤ 13 Safari) would see a broken image if they request that file directly — a small and shrinking slice of traffic. Turn off next-gen format conversion in Settings if you need to guarantee support for them; Squoosh will still recompress in the original format.
...
`build/` is git-ignored and generated by `@wordpress/scripts` (webpack). It ships in the distributed zip (see `.distignore`) but is rebuilt in CI on release. Two entry points are built: `index.js` (the Media → Squoosh dashboard) and `auto-upload.js` (on-upload optimization + Media row actions).

## Structure

```
onylogy-squoosh/
├── onylogy-squoosh.php   # main file: headers, constants, activation
├── includes/
