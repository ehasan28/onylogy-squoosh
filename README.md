...
By **Ehasanul Haque** ([@ehasan28](https://github.com/ehasan28)) · GPL-2.0-or-later
...
- records savings in the `_onyio_data` post meta,
...
`build/` is git-ignored and generated by `@wordpress/scripts` (webpack). It ships in the distributed zip (see `.distignore`) but is rebuilt in CI on release. Two entry points are built: `index.js` (the Media → Image Optimizer dashboard) and `auto-upload.js` (on-upload optimization + Media row actions).

## Structure

```
onylogy-squeeze-wp/
├── onylogy-image-optimizer.php # main file: headers, constants, activation
├── includes/
...
│   └── class-rest.php          # onyio/v1 REST API (queue/store/complete/restore/stats/settings)
