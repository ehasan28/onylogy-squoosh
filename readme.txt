...
Compress, resize and convert your Media Library to WebP/AVIF — all in your browser via WASM. No cloud, no API keys, no per-image cost.
...
* **Resize oversized images** — off by default; when turned on, downscale huge uploads to a sensible max width (up to 2560px). Never upscales.
...
* Optimize/Restore now available directly on Grid view thumbnails (previously List view only).
...
* Resize is now off by default (previously always capped at 2560px); when enabled, the top preset is labeled Max (2560px).
